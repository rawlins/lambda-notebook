...
class TypedExpr(object):
    ...
    def __init__(self, op, *args):
        ...
        if (op in basic_ops):
            ...
            print("Warning: constructing expression from deprecated basic operator '%s'" % op)
            ...
            self.op = op
            #if op.type != unify_f:
            #    print("Warning: unify suggested a strengthened LFun type, but could not accommodate: %s -> %s" % (op.type, unify_f))
            if unify_a != arg.type:
            ...
            if (len(args) > 1):
                ...
                raise TypeMismatch(op, args, "Term+arg expression (too many arguments)")
            elif (len(args) == 0):
                self.type = op.type
                self.op = op
                self.args = list()
                ...
                unify_f, unify_a, unify_r = type_sys.local_unify_fa(op.type, arg.type)
                if unify_f is None:
                    raise TypeMismatch(op, arg, "Term+arg expression (unification failed)")
                #print(op.type, arg.type, unify_f, unify_a, unify_r)
                self.type = unify_r
                #print(self.type)
                if op.type == unify_f:
                    self.op = op
                else:
                    if isinstance(op, TypedTerm):
                        # only strengthen terms.  TODO: this is not very general.  For example, "[[NP]](x_e)" as op.
                        self.op = op.copy()
                        self.op.type = unify_f
                    else:
                        print("Warning: unify suggested a strengthened op type, but could not accommodate: %s -> %s" % (op.type, unify_f))
                        self.op = op
                if unify_a != arg.type:
                    if isinstance(arg, TypedTerm):
                        arg = arg.copy()
                ...
                self.args = [arg]
            ######## old code pre-unification...do not delete until really sure!
            # if not op.type.functional():
            #     if len(args) == 1:
            #         get_type_system().fun_arg_check(op, args[0])
            #         # if op.type.undetermined or args[0].type.undetermined:
            #         #     if not types.UndeterminedType.fun_arg_ok(op, args[0]):
            #         #         raise TypeMismatch(op, args[0], "Term+arg expression")
            #         #     # otherwise, expression is typed ok as far as we can currently tell.
            #         # else:
            #         #     # non-functional operator receives an argument.
            #         #     raise TypeMismatch(op, args[0], "Term+arg expression")
            #     elif len(args) > 1:
            #         # multiple arguments.
            #         raise TypeMismatch(op, args, "Term+arg expression")
            #     # is this ever reached??
            #     # A: it is now.  Should only be reached if the undetermined-type fun/arg checking is satisfied.
            #     self.op = op
            #     self.args = list(args)
            #     self.type = op.type
            # else: 
            #     # op.type is functional
            #     self.op = op
            #     # TODO: implement n-ary predicates somehow
            #     if len(args) > 0:
            #         arg = self.ensure_typed_expr(args[0])
            #         arg.type_not_guessed()
            #         if len(args) > 1 or arg.type != op.type.left:
            #             raise TypeMismatch(op, arg, "Term+arg expression")
            #         self.args = [arg]
            #         self.type = op.type.right # term is saturated, type of whole is output type
            #     else:
            #         # term is unsaturated
            #         # again, is this ever reached??
            #         self.args = list()
            #         self.type = op.type # use unsaturated type
    ...
    def try_adjust_type(self, new_type):
        ...
        if self.type == unify_a:
            ...
            if isinstance(self.op, TypedExpr):
                ...
                new_op = self.op.try_adjust_type(new_op_type)
                if new_op is None:
                    return None
                self_copy = self.copy()
                self_copy.op = new_op
    ...
    def parse_expr_string(cls, s, assignment=None):
        ...
        return eval(s, {'TypedExpr':TypedExpr,'TypedTerm':TypedTerm, 'assignment': assignment, 'type_e': type_e})
    ...
    def try_parse_op_expr(cls, s, assignment=None):
        ...
        ops = {"lambda " : "L", "L " : "L", "λ" : "L", "Forall " : "A", "Exists " : "E"}
        next = None
        op = None
        for k in ops.keys():
            if s.startswith(k):
    ...
    def find_term_locations(cls, s, i=0):
        ...
        return parsing.find_pattern_locations(term_re, s, i)
        ...
        if (assignment is not None):
            if typ is None:
                ...
                if term_name.group(1) in assignment:
                    ...
                    if u_l is None:
                        raise TypeMismatch(term_name.group(1), assignment[term_name.group(1)], "Variable type inheritence")
                    else:
                        # TODO: better logic?
                        typ = u_l
    ...
    @classmethod
    def ensure_typed_expr(cls, s, typ = None):
    ...
    def reduce(self):
        ...
        if len(self.args) == 0:
            ...
            return self.op.apply(self.args[0])
        else:
    ...
    def reduce_all(self):
        ...
        if isinstance(result.op, TypedExpr):
            new_op = result.op.reduce_all()
            if new_op is not result.op and not dirty:
                dirty = True
                result = result.copy()
            result.op = new_op
        for i in range(len(result.args)):
            new_arg_i = result.args[i].reduce_all()
            if new_arg_i is not result.args[i] and not dirty:
                dirty = True
                result = result.copy()
...
class TypedTerm(TypedExpr):
    ...
    def __init__(self, varname, typ=None):
        self.op = varname
        if typ is None:
    ...
    def latex_str(self):
        if suppress_constant_type and self.constant():
            return ensuremath("{%s}" % self.op)
        else:
            return ensuremath("{%s}_{%s}" % (self.op, self.type.latex_str()))
...
class BinaryOpExpr(TypedExpr):
    ...
    Because of the way the copy function works, it is currently not suited for direct instantiation."""
    def __init__(self, typ, op, arg1, arg2, op_name_uni=None, op_name_latex=None):
        self.op = op
...
def binary_num_rel(op, op_uni=None, op_latex=None):
    ...
    class BinOp(BinaryOpExpr):
        def __init__(self, arg1, arg2):
            ...
            super().__init__(type_t, op, self.ensure_typed_expr(arg1, types.type_n), self.ensure_typed_expr(arg2, types.type_n), op_uni, op_latex)
    return BinOp
...
class BindingOp(TypedExpr):
    ...
    def __init__(self, vartype, typ, op, var, body, op_name_uni=None, op_name_latex=None, body_type = None):
        ...
        self.args = [self.ensure_typed_expr(body, body_type)]
...
class LFun(BindingOp):
    ...
    def try_adjust_type(self, new_type):
        ...
        if self.type == unify_a:
            ...
            new_argtype = unify_a.left
            if self.argtype != unify_a.left:
                # arg type needs to be adjusted, and hence all instances of the bound variable as well.  Do this with beta reduction.
                new_var = TypedTerm(self.varname, unify_a.left)
                new_body = self.apply(new_var)
            else:
                new_body = self.body
            #if new_body.type != unify_a.right:
            new_body = new_body.try_adjust_type(unify_a.right) # will only make copy if necessary
